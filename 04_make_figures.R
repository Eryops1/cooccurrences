...
rm(shapes, grid_sub_cz, grid_sub_eu, grid_sub_ny, grid_sub_nz, m)
...
chan = unique(fin[,.(over, species_pair, atlas, scaleID, 
                     delta_cor_obs, delta_c_obs, delta_c_obs_z, delta_cor_obs_z, cor_obs_T1)])
chan = chan[over==TRUE,]
...
# get average vlaues for individual species from all pairs
...
range_change$atlas = gsub(5, "Czechia", range_change$atlas)
range_change$atlas = gsub("26", "Europe", range_change$atlas)
range_change$atlas = gsub("6", "New York", range_change$atlas)
range_change$atlas = gsub("17", "New Zealand", range_change$atlas)

indiv = merge(indiv, range_change[,.(species, atlas, change_occupancy, change_pres.abs, pres.abs_sum, mean.psi_sum)], by=c("atlas", "species"), 
              all.x=TRUE)
...
## general range size within Primary Lifestyle groups (n occupied cells) ----

ggplot(indiv, aes(y=Primary.Lifestyle, x=pres.abs_sum, fill=atlas))+
...
avonet$scientificName = avonet$species
range = merge(range_change, 
              avonet[,.SD, .SDcols = !c('Mass.Refs.Other', 'Reference.species', 'Traits.inferred')],
